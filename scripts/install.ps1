    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "$ArchiveBaseUrl/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "$ArchiveBaseUrl/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "$ArchiveBaseUrl/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                # Refs commonly contain '/', which is valid in the archive URL
                # but not in a flat Windows temp filename.
