...
def _apply_default_agent_settings(config: dict):
    ...
    config["compression"]["threshold"] = 0.70
    ...
    print_info("  Compression threshold: 0.70")
...
def setup_agent_settings(config: dict):
    ...
    current_threshold = cfg_get(config, "compression", "threshold", default=0.70)
    ...
    print_success(
        f"Context compression threshold set to {config['compression'].get('threshold', 0.70)}"
    )
