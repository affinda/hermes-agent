...
def _resolve_git_url(repo: str) -> str:
    """Turn ``owner/repo`` into an SSH clone URL; pass full URLs through.

    SSH is the default so the marketplace pulls reuse the same key the fork's
    ``git pull`` already relies on (the fork tracks ``git@github.com:affinda/...``).
    """
